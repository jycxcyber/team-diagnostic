...
# ==========================================
# VIEW 2: ADMINISTRATOR DASHBOARD (Multi-User & Audit Logging Enabled)
# ==========================================
elif view_mode == "📊 Administrator Dashboard":
    st.header("Admin Analytics & Security Center")
    
    # 1. Define Multiple Admin Credentials Database
    # In production, replace this with a real database or Streamlit Secrets!
    ADMIN_CREDENTIALS = {
        "admin_jacky": "JackySecure2026!",
        "admin_kammy": "Kammy789*",
        "director_jacky": "JackyMasterKey#1"
    }
    
    # Initialize session states so Streamlit remembers who is logged in
    if "admin_authenticated" not in st.session_state:
        st.session_state["admin_authenticated"] = False
    if "current_admin_user" not in st.session_state:
        st.session_state["current_admin_user"] = None
    ...
    if not st.session_state["admin_authenticated"]:
        ...
        login_submitted = st.sidebar.button("Login")
        
        if login_submitted:
            # Check if username exists and password matches
            if input_user in ADMIN_CREDENTIALS and ADMIN_CREDENTIALS[input_user] == input_password:
                st.session_state["admin_authenticated"] = True
                st.session_state["current_admin_user"] = input_user
                
                # Write SUCCESS to the audit trail log
                eh.log_admin_action(input_user, "Dashboard Login", "Success")
                st.rerun()
            else:
                # Write FAILURE to the audit trail log if the username was typed in
                target_user = input_user if input_user.strip() else "Unknown User"
                eh.log_admin_action(target_user, "Attempted Unauthorized Login", "Failed")
                st.sidebar.error("❌ Invalid Username or Password combination.")
                
        st.warning("🔒 Access Restricted. Please input valid credentials in the sidebar portal.")
        
    else:
        # 3. Authenticated View
        current_user = st.session_state["current_admin_user"]
        st.sidebar.success(f"👤 Active Session: {current_user}")
        
        # Logout Button
        if st.sidebar.button("Log Out"):
            eh.log_admin_action(current_user, "User Initiated Logout", "Success")
            st.session_state["admin_authenticated"] = False
            st.session_state["current_admin_user"] = None
            st.rerun()
            
        # --- MAIN DASHBOARD INTERFACE CONTINUES BELOW ---
        df_res = eh.load_responses()
        
        # Tab layouts: One for analytics, one for security audits
        tab_analytics, tab_security = st.tabs(["📊 Performance Analytics", "📜 Access Audit Logs"])
        
        with tab_analytics:
            # Insert your entire existing Radar Graph and Insights generation code here...
            st.write(f"Welcome back, **{current_user}**. Here are the current metrics:")
            
            # (Keep your existing radar chart and analytics code running smoothly inside here)
            # ... [Existing data filtration, Radar Chart code, and Insight generation goes here] ...
            
        with tab_security:
            st.subheader("🕵️‍♂️ System Access Audit Trail")
            st.markdown("This log records all real-time administrator login and logout activities directly from the database.")
            
            try:
                # Load and read the current log entries straight out of Excel
                df_audit_logs = pd.read_excel("team_diagnostic.xlsx", sheet_name="AdminLogs")
                # Sort newest first
                df_audit_logs = df_audit_logs.sort_index(ascending=False)
                st.dataframe(df_audit_logs, use_container_width=True)
            except Exception:
                st.info("No logs found. The audit ledger is currently empty.")
